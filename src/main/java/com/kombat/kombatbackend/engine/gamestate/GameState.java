...
public class GameState {
    ...
    public GameState(Board board,
        ...
        if (initialMinions == null) throw new NullPointerException("Minion list cannot be null");
        this.minions = initialMinions;

        this.turnNumber = 0;

        // default (phase-2) owner
        kindsByOwner.put(0L, new EnumMap<MinionType, MinionKindDef>(MinionType.class));
        checkRep();
    }

    public Board getBoard() { return board; }
    public List<Minion> getMinions() { return Collections.unmodifiableList(minions); }
    ...
    public void registerKind(MinionKindDef def) { registerKind(0L, def); }

    public void registerKind(long ownerId, MinionKindDef def) {
        Objects.requireNonNull(def, "def must not be null");
        if (setupLocked) throw new IllegalStateException("Setup is locked; cannot register new kinds");

        EnumMap<MinionType, MinionKindDef> kinds = kindsByOwner.computeIfAbsent(
                ownerId,
                k -> new EnumMap<MinionType, MinionKindDef>(MinionType.class)
        );

        MinionType key = def.getType();
        if (kinds.containsKey(key)) throw new IllegalArgumentException("Duplicate minion type: " + key);
        kinds.put(key, def);
    }

    public Map<MinionType, MinionKindDef> getKinds() { return getKinds(0L); }
    public Map<MinionType, MinionKindDef> getKinds(long ownerId) {
        EnumMap<MinionType, MinionKindDef> kinds = kindsByOwner.get(ownerId);
        if (kinds == null) return Collections.emptyMap();
        return Collections.unmodifiableMap(kinds);
    }

    public MinionKindDef getKind(MinionType type) { return getKind(0L, type); }
     ...
     * - else if name looks like a type (Fighter/Assasin/...) -> return that type's kind
     */
    public MinionKindDef getKind(String name) { return getKind(0L, name); }
    ...
    public MinionKindDef getKind(long ownerId, String name) {
        ...
        for (MinionKindDef def : kinds.values()) {
            if (def != null && def.getKindName() != null && def.getKindName().equalsIgnoreCase(q)) {
                return def;
            }
        }
        try {
    ...
    public void lockSetup() { this.setupLocked = true; }
    public boolean isSetupLocked() { return setupLocked; }
    ...
    public Minion spawnMinion(long ownerId, String kindOrType, int initialHp, int x, int y) {
        ...
        if (def == null) throw new IllegalArgumentException("Unknown minion kind/type: " + kindOrType);
    ...
    public Minion spawnMinion(long ownerId, MinionType type, int x, int y) {
        ...
        if (hp > Integer.MAX_VALUE) throw new IllegalStateException("init_hp too large for int: " + hp);
    ...
    public Minion spawnMinion(long ownerId, MinionType type, int initialHp, int x, int y) {
        MinionKindDef def = getKind(ownerId, type);
        if (def == null) throw new IllegalArgumentException("Unknown minion type for ownerId=" + ownerId + ": " + type);

        if (!board.isInsideBoard(x, y)) {
            throw new IllegalArgumentException("Spawn position outside board: x=" + x + ",y=" + y);
        }

        Hex h = board.getHex(x, y);
        if (h.isOccupied()) {
            throw new IllegalStateException("Spawn position already occupied: x=" + x + ",y=" + y);
        }

        // ราคาเท่ากันทุก type: spawn_cost
        long cost = config.spawnCost();
        if (cost > 0) budgetManager.spendBudget(ownerId, cost);

        Minion m = new Minion(initialHp, def.getDefenseFactor(), h, def.getStrategy(), ownerId);
        m.setType(type);
    ...
    public Map<Minion, Strategy> buildStrategyBindings(long ownerId) {
        IdentityHashMap<Minion, Strategy> map = new IdentityHashMap<Minion, Strategy>();
        for (Minion m : minions) {
            if (m == null) continue;
            if (m.getOwnerId() != ownerId) continue;
            Object ref = m.getStrategyRef();
            if (ref instanceof Strategy) map.put(m, (Strategy) ref);
        }
        return map;
    }
    ...
    public boolean removeMinion(Minion m) {
        ...
        // Remove from board if it is still occupying its position.
        Hex pos = m.getPosition();
        if (pos != null) {
            if (pos.getOccupant() == m) {
                pos.removeMinion();
            } else if (board.isInsideBoard(pos.getX(), pos.getY())) {
                // Defensive: if the Minion holds a stale Hex reference, try by coordinates.
                Hex h = board.getHex(pos.getX(), pos.getY());
                if (h.getOccupant() == m) h.removeMinion();
            }
        }
    ...
    public void advanceTurn() {
        if (turnNumber < 0) throw new IllegalStateException("Turn number must be non-negative");
        turnNumber++;
        checkRep();
    }
    ...
    private void checkRep() {
        if (turnNumber < 0) throw new IllegalStateException("Turn number must be non-negative");
        if (phase == null) throw new IllegalStateException("Phase must not be null");

        for (Minion m : minions) {
            if (m == null) throw new IllegalStateException("Minion list must not contain null");
            Hex pos = m.getPosition();
            if (pos == null) throw new IllegalStateException("Minion position must not be null");
            if (!board.isInsideBoard(pos.getX(), pos.getY())) throw new IllegalStateException("Minion is outside the board");
            Hex boardHex = board.getHex(pos.getX(), pos.getY());
            if (boardHex.getOccupant() != m) throw new IllegalStateException("Board and minion position mismatch");
        }

        for (Map.Entry<Long, EnumMap<MinionType, MinionKindDef>> ownerEntry : kindsByOwner.entrySet()) {
            if (ownerEntry.getKey() == null || ownerEntry.getValue() == null) {
                throw new IllegalStateException("Kinds must not contain null ownerId/maps");
            }
            EnumMap<MinionType, MinionKindDef> kinds = ownerEntry.getValue();
            for (Map.Entry<MinionType, MinionKindDef> e : kinds.entrySet()) {
                if (e.getKey() == null || e.getValue() == null) {
                    throw new IllegalStateException("Kinds must not contain null keys/values");
                }
                if (e.getKey() != e.getValue().getType()) {
                    throw new IllegalStateException("Kinds key/type mismatch: " + e.getKey());
                }
            }
        }
    }
